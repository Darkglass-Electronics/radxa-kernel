...
static struct usb_string strings_fn[5] = {
	[STR_FU_OUT].s = "Volume",
	/* [STR_ASSOC].s = DYNAMIC, */
...
afunc_bind(struct usb_configuration *cfg, struct usb_function *fn)
{
	...
	ret = usb_assign_descriptors(fn, NULL, hs_audio_desc, ss_audio_desc,
				     ss_audio_desc);
	if (ret)
		goto err_free_fu;
	...
	if (entity_id == USB_CLK_ID) {
		...
		if (control_selector == UAC_FU_MUTE) {
			...
			*(u8 *)req->buf = mute;
			...
			u_audio_get_volume(agdev, &volume);
...
out_rq_cur_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (entity_id == USB_OUT_FU_ID) {
		if (control_selector == UAC_FU_MUTE) {
			u8 mute = *(u8 *)req->buf;
			u_audio_set_mute(agdev, mute);
			return;
			...
			u_audio_set_volume(agdev, volume);
			return;
